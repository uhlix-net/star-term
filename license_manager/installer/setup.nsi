!define APPNAME "star_term_license_manager"
!define DISPLAYNAME "Star Term License Manager"
...
Name "${APPNAME}"
OutFile "Output\license_manager_setup.exe"
...
Function .onInit
  ...
  ; Bring the installer window to the front so it doesn't end up hidden
...
Section "Application" SecApp
  ...
  SetOutPath "$INSTDIR\license_manager"
  File "..\license_manager\*.py"

  SetOutPath "$INSTDIR"
  File "..\requirements.txt"
  File "run_license_manager.bat"
  File "app.ico"
  ...
  CreateShortcut "$SMPROGRAMS\${DISPLAYNAME}\${DISPLAYNAME}.lnk" "$INSTDIR\run_license_manager.bat" "" "$INSTDIR\app.ico" 0
  CreateShortcut "$SMPROGRAMS\${DISPLAYNAME}\Uninstall ${DISPLAYNAME}.lnk" "$INSTDIR\Uninstall.exe" "" "" 0
  CreateShortcut "$DESKTOP\${DISPLAYNAME}.lnk" "$INSTDIR\run_license_manager.bat" "" "$INSTDIR\app.ico" 0
