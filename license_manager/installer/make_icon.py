"""Generates installer/app.ico from the key icon used for the application's
window/taskbar icon, so the Start Menu/Desktop shortcuts match."""
...
from license_manager import icons
...
def main():
    ...
    images = []
    for size in SIZES:
        pm = icons.key_icon(size).pixmap(size, size)
